...
let enabled = true;
const items = 9;
const wheels = [1, 2, 3].map((x) =>
  document.querySelector(`#dropdown .dropdown .wheel${x}`)
);
...
const round_and_modulus_numbers = (numbers, mod) =>
  numbers.map((x) => Math.round(x) % items);
const minimum_spins = 3;
const state_increment = (current, index) =>
  (minimum_spins + Math.random()) *
    items * //minimum spins with 0-1 extra
    ((index + 2) / 3) +
...
const background_position_value = (position) =>
  Math.round(position) * row_height;
const background_position = (position) =>
  `${background_position_value(position)}px`;
...
const spin = (state) => {
  ...
  wheels.forEach((el, i) => {
    el.style.backgroundPositionY = background_position(next_state[i]);
  });
